...
public class ReportGenerator {
  ...
  public String generateHistoricalReport(CurrencyList historical, String fileName, String base,
  ...
  public Path generateLatestReport(CurrencyList latest, String fileName, String base)
      throws IOException {

    String resultFileName =
        createFileName(fileName + "_" + base, LocalDate.now(ZoneId.of(TIME_ZONE)).toString());
    Files.createDirectories(REPORT_DIRECTORY);
    Path reportPath = REPORT_DIRECTORY.resolve(resultFileName).normalize();
  ...
  public void generateExcelReport(Path reportPath, Map<String, BigDecimal> currToRateOfExchange,
    ...
    try {
      for (Entry<String, BigDecimal> entry : currToRateOfExchange.entrySet()) {
        
        Row row = currenciesSheet.createRow(rCounter++);
        addValuesToRow(entry.getKey(), entry.getValue(), row, cCounter, base);
